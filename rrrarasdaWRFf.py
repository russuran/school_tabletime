...
def reminder_message(message):
    ...
    bot.send_message(message.chat.id,"Выберите датуㅤㅤㅤㅤㅤ",reply_markup=calendar.create_calendar(name=calendar_2_callback.prefix,year=now.year,month=now.month,),)



def reminder_set_time(message, date):
    bot.send_message(message.chat.id, 'Введите время (часы:минуты)')
    bot.register_next_step_handler(message, reminder_set_name, date)


def reminder_set_name(message, date):
    bot.send_message(message.chat.id, 'Введите текст для напоминания')
    bot.register_next_step_handler(message, reminder_set, date, message.text)


def reminder_set(message, date, time):
    global user_data
    try:
        ...
        reminder_time = datetime.datetime(int(date[2]), int(date[1]), int(date[0]), int(time[0]), int(time[1]))
        now = datetime.datetime.now()
...
def callback_inline(call: CallbackQuery):
        ...
        if action == "DAY":
            ...
            reminder_set_time(call.message, date.strftime('%d.%m.%Y'))
...
def buildMainMenu(message, name=''):
    ...
    item2 = types.InlineKeyboardButton('🏛 Прочие функции', callback_data='other')
    markup.row(item2)
...
def buildOtherMenu(call, name=''):
    ...
    item3 = types.InlineKeyboardButton('⏳ Напоминания (в разработке)', callback_data='checkTimes')
    markup.row(item3)
